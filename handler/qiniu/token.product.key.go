...
type TokenProductRequest struct {
	Token  string `form:"token" json:"token" binding:"required"`
	Amount uint   `form:"amount" json:"amount" binding:"required"`
}

func TokenProductHandler(c *gin.Context) {
	...
	for i < req.Amount {
		upToken, key, link := qiniu.UpToken(Config.Qiniu, fmt.Sprintf("%s/%s/%d", Config.Qiniu.TokenProductImagePath, req.Token, i), timestamp)
